...
function getPoemSearchQuery(poem: PoemLine) {
  return [poem.origin?.title, poem.origin?.author, poem.content]
    .filter(Boolean)
    .join(" ");
}
...
export function PoemDisplay({
  ...
  useLayoutEffect(() => {
    ...
    const updateTextWidths = () => {
      const nextWidths = {
        collapsed: Math.ceil(
          (collapsedTextMeasureRef.current?.offsetWidth ?? 0) +
            TEXT_WIDTH_TOLERANCE,
        ),
        expanded: Math.ceil(
          (expandedTextMeasureRef.current?.offsetWidth ?? 0) +
            TEXT_WIDTH_TOLERANCE,
        ),
      };
      ...
      setTextWidths((currentWidths) => {
        if (
          currentWidths.collapsed === nextWidths.collapsed &&
          currentWidths.expanded === nextWidths.expanded
        ) {
  ...
  const extraCompactThreshold =
    titleViewportWidth > 0 ? titleViewportWidth - 96 : 0;
  const headSizeClass =
  ...
  const handleCopyPoem = async () => {
    await handleCopyText(
      formatPoemText(poem, poemLines, settings.showPoemDynasty),
    );
  };
    ...
    <>
      ...
      >
          ...
          >
            ...
            <span
              className={cn(
                ...
                poemLifted &&
                  "group-hover/poem:pointer-events-auto group-hover/poem:opacity-100",
              )}
        ...
        <span className="pointer-events-none absolute opacity-0">
          <span
            ref={collapsedTextMeasureRef}
            className="inline-block whitespace-nowrap [font:inherit]"
          >
            {poem.content}
          </span>
          <span
            ref={expandedTextMeasureRef}
            className="inline-block whitespace-nowrap [font:inherit]"
          >
            {title || poem.content}
          </span>
